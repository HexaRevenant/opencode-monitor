...
`npm run typecheck`, and `npm run build`, in that order. Package installation
examples elsewhere in the README MUST match the version declared by
`package.json`.
...
#### Scenario: Version examples match the package

- GIVEN `package.json` declares a package version
- WHEN package installation examples are shown in the README
- THEN those examples use the same declared version
